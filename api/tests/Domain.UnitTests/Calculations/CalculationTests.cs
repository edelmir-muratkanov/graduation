...
public class CalculationTests
{
    ...
    [Fact]
    public void UpdateItem_Should_ReturnSuccessResult_WhenItemPresent()
    {
        var calculation = Domain.Calculation.Calculation.Create(Guid.NewGuid(), Guid.NewGuid());
        const string propertyName = "property_name";
        calculation.AddItem(propertyName, new Belonging(1));

        Result result = calculation.UpdateItem(propertyName, new Belonging(0.5));

        result.IsSuccess.Should().BeTrue();
    }

    [Fact]
    public void UpdateItem_Should_ReturnFailureResult_WhenItemAbsent()
    {
        var calculation = Domain.Calculation.Calculation.Create(Guid.NewGuid(), Guid.NewGuid());
        const string propertyName = "property_name";
        calculation.AddItem(propertyName, new Belonging(1));

        Result result = calculation.UpdateItem("some_item", new Belonging(0.5));
